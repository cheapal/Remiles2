...
void main() {
  runApp(const MaterialApp(
    ...
    home: ManageLoadsCancelled(),
  ));
}

class ManageLoadsCancelled extends StatefulWidget {
  const ManageLoadsCancelled({super.key});

  @override
  State<ManageLoadsCancelled> createState() => _ManageLoadsCancelledState();
}

class _ManageLoadsCancelledState extends State<ManageLoadsCancelled>
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        _buildPrimaryButton(
                            '+ Post Loads', const Color(0xFFFFCF5F), Colors.black),
                    ...
                    // Status buttons (Cancelled active)
                    Row(
                      children: [
                        _buildLoadsButton('Active Loads', false, context),
                        _buildLoadsButton('In-Transit', false, context),
                        _buildLoadsButton('Cancelled Loads', true, context),
                        _buildLoadsButton('Completed Loads', false, context),
                      ],
                    ),
                    const SizedBox(height: 20),

                    // Cards – same look as Manage_loads_3
                    _buildLoadCard(context, status: 'Cancelled'),
  ...
  /// CARD — same look/feel as in Manage_loads_3,
  /// but with Cancelled + Repost on the right.
  Widget _buildLoadCard(BuildContext context, {required String status}) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(16),
      margin: const EdgeInsets.symmetric(horizontal: 2), // tiny breathing room so shadow isn't clipped
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(26),
        boxShadow: [
          BoxShadow(
            color: const Color(0xFF6CA78A).withOpacity(0.2), // same greenish soft shadow
            blurRadius: 13.4,
            spreadRadius: 2,
            offset: const Offset(0, 3),
          ),
          ...
          Row(
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  _buildDetailRow(
                      icon: Icons.location_on, text: 'From : Toronto, ON'),
                  const SizedBox(height: 5),
                  _buildDetailRow(
                      icon: Icons.location_on, text: 'To : Montreal. QC'),
                  const SizedBox(height: 5),
                  _buildDetailRow(
              ...
              // right side status + id + equipment (matches previous layout)
              Column(
    ...
    return Row(
      children: [
        ...
        Text(
          text,
          style: const TextStyle(
              fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF195529)),
        ),
      ],
    );
  }
